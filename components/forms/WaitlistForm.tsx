...
export function WaitlistForm({ source = 'homepage', className }: WaitlistFormProps) {
  ...
  if (status === 'success') {
    return (
      <div className={cn('border border-rule p-10 text-center', className)} role="status">
  ...
  return (
    <form onSubmit={handleSubmit} className={cn('space-y-6', className)} noValidate>
      ...
      <button type="submit" className="cta-btn w-full justify-center md:w-auto" disabled={status === 'loading'}>
